function [fh,xh,gh,H,itct,fcount,retcodeh] = mycsmin(fcn,x0,H0,grad,crit,nit,varargin);
...
if NumGrad
   if length(grad)==0
      [g badg] = mynumgrad(fcn,x0, varargin{:});
      %ARGLIST
      %[g badg] = mynumgrad(fcn,x0,P1,P2,P3,P4,P5,P6,P7,P8,P9,P10,P11,P12,P13);
   else
...
while ~done
   ...
   %disp(sprintf('iteration %d : f  %20.10f',itct,f))
   %-----------Comment out this line if the x vector is long----------------
   %   disp([sprintf('x = ') sprintf('%15.8g',x)]);
   %-------------------------
   itct=itct+1;
   [f1 x1 fc retcode1] = mycsminit(fcn,x,f,g,badg,H,varargin{:});
   %ARGLIST
   %[f1 x1 fc retcode1] = mycsminit(fcn,x,f,g,badg,H,P1,P2,P3,P4,P5,P6,P7,...
   ...
   if retcode1 ~= 1
      if retcode1==2 | retcode1==4
         ...
         if NumGrad
            [g1 badg1] = mynumgrad(fcn, x1,varargin{:});
            %ARGLIST
            %[g1 badg1] = mynumgrad(fcn, x1,P1,P2,P3,P4,P5,P6,P7,P8,P9,...
            %                P10,P11,P12,P13);
         else
...
%         disp('Cliff.  Perturbing search direction.')
         [f2 x2 fc retcode2] = mycsminit(fcn,x,f,g,badg,Hcliff,varargin{:});
         %ARGLIST
         %[f2 x2 fc retcode2] = mycsminit(fcn,x,f,g,badg,Hcliff,P1,P2,P3,P4,...
         ...
         if  f2 < f
            if retcode2==2 | retcode2==4
               ...
               if NumGrad
                  [g2 badg2] = mynumgrad(fcn, x2,varargin{:});
                  %ARGLIST
                  %[g2 badg2] = mynumgrad(fcn, x2,P1,P2,P3,P4,P5,P6,P7,P8,...
                  %      P9,P10,P11,P12,P13);
               else
...
%               disp('Cliff again.  Try traversing')
               if norm(x2-x1) < 1e-13
                  ...
                  [f3 x3 fc retcode3] = mycsminit(fcn,x,f,gcliff,0,eye(nx),varargin{:});
                  %ARGLIST
                  %[f3 x3 fc retcode3] = mycsminit(fcn,x,f,gcliff,0,eye(nx),P1,P2,P3,...
                  %         P4,P5,P6,P7,P8,...
                  %      P9,P10,P11,P12,P13);
                  fcount = fcount+fc; % put by Jinill
                  if retcode3==2 | retcode3==4
                     ...
                     if NumGrad
                        [g3 badg3] = mynumgrad(fcn, x3,varargin{:});
                        %ARGLIST
                        %[g3 badg3] = mynumgrad(fcn, x3,P1,P2,P3,P4,P5,P6,P7,P8,...
                        %                        P9,P10,P11,P12,P13);
                     else
     ...
     %disp(sprintf('ih = %d',ih))
      ...
      if nogh
         if NumGrad
            ...
            [gh badgh] = feval('mynumgrad',fcn,xh,varargin{:});
         else
