function [hess,err] = hessian_sng(fun,x0,varargin)
...
% modified by S. Ng to allow for additional input arguments
...
[hess,err] = hessdiag_sng(fun,x0,varargin{:});
...
[grad,graderr,stepsize] = gradest_sng(fun,x0,varargin{:});
  ...
  for j = 1:(i-1)
    ...
    for k = 1:(params.RombergTerms+1)
      dij(k) = fun(x0 + swap2(zeros(sx),i, ...
        dfac(k)*stepsize(i),j,dfac(k)*stepsize(j)),varargin{:}) + ...
