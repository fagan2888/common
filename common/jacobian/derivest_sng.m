function [der,errest,finaldelta] = derivest_sng(fun,x0,varargin)
...
if (rem(par.DerivativeOrder,2) == 0) || ~strncmpi(par.Style,'central',7)
  if strcmpi(par.Vectorized,'yes')
    f_x0 = fun(x0,varargin{:});
  else
    % not vectorized, so loop
    f_x0 = zeros(size(x0));
    for j = 1:numel(x0)
      f_x0(j) = fun(x0(j),varargin{:});
    end
  end
else
  f_x0 = [];
...
for i = 1:n
  ...
  if par.Style(1) == 'c'
    ...
    if strcmpi(par.Vectorized,'yes')
      f_plusdel = feval(fun(x0i+h*delta),varargin{:});
      f_minusdel = fevak(fun(x0i-h*delta),varargin{:});
    else
      % not vectorized, so loop
      f_minusdel = zeros(size(delta));
      f_plusdel = zeros(size(delta));
      for j = 1:numel(delta)
        f_plusdel(j) = feval(fun(x0i+h*delta(j)),varargin{:});
        f_minusdel(j) = feval(fun(x0i-h*delta(j)),varargin{:});
      end
    end
  ...
  elseif par.Style(1) == 'f'
    ...
    if strcmpi(par.Vectorized,'yes')
      f_del = feval(fun(x0i+h*delta),varargin{:}) - f_x0(i);
    else
      % not vectorized, so loop
      f_del = zeros(size(delta));
      for j = 1:numel(delta)
        f_del(j) = feval(fun(x0i+h*delta(j)),varargin{:}) - f_x0(i);
      end
    end
  else
    % backward rule
